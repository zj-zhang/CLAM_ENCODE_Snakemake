...
create_df_names <- function(gr, filename, record.names=NULL){
  ...
  df <- unique(df)
  df$starts = as.character(format(df$starts, trim=T, scientific=F))
  df$ends = as.character(format(df$ends, trim=T, scientific=F))
...
proximal200_df <- data.frame(seqnames=rep( seqnames(all.exons), 2 ),
                 starts=c( start(all.exons)-201, end(all.exons)),
                 ends=c( start(all.exons), end(all.exons)+200 ),
                 names= rep( gsub('\\.[0-9]+', '', names(all.exons)) , 2) ,
                 scores=c(rep('.', length(all.exons)*2)),
                 strands=rep( strand(all.exons), 2)  )
proximal200_df <- arrange(proximal200_df, seqnames, starts, ends, names)
proximal200_df <- unique(proximal200_df)
proximal200_df <- subset(proximal200_df, proximal200_df$starts>0)
proximal200_df <- subset(proximal200_df, proximal200_df$ends>0)
proximal200_df$starts =  as.character(format(proximal200_df$starts, trim=T, scientific=F))
proximal200_df$ends =  as.character(format(proximal200_df$ends, trim=T, scientific=F))
write.table(proximal200_df, file=file.path(output_dir, "proximal200_intron.bed"), quote=F, sep='\t', row.names=F, col.names=F)


proximal500_df <- data.frame(seqnames=rep( seqnames(all.exons), 2 ),
                 starts= c( start(all.exons)-501, end(all.exons) ),
                 ends= c( start(all.exons), end(all.exons)+500 ),
                 names= rep( gsub('\\.[0-9]+', '', names(all.exons)) , 2) ,
                 scores=c(rep('.', length(all.exons)*2)),
                 strands=rep( strand(all.exons), 2)  )
proximal500_df <- arrange(proximal500_df, seqnames, starts, ends, names)
proximal500_df <- unique(proximal500_df)
proximal500_df <- subset(proximal500_df, proximal500_df$starts>0)
proximal500_df <- subset(proximal500_df, proximal500_df$ends>0)
proximal500_df$starts =  as.character(format(proximal500_df$starts,trim=T,scientific=F))
proximal500_df$ends =  as.character(format(proximal500_df$ends,trim=T,scientific=F))
write.table(proximal500_df, file=file.path(output_dir, "proximal500_intron.bed"), quote=F, sep='\t', row.names=F, col.names=F)
